...
exports.createPromiseTsAst = (interfaceChunkGather) => {
    const tplName = `${MarkDown_1.INTERFACE_NAME_PREFIX}${firstWordUpperCase_1.firstWordUpperCase(MODULE_NAME)}${MarkDown_1.RESPONSE_NAME_SUFFIX}`;
    const tplBody = [];
    interfaceChunkGather.forEach(value => {
        const singleBody = objDeepCopy_1.objDeepCopy(interfacePromiseTsAsTpl_1.interfacePromiseAst.declaration.body.body[0]);
    ...
    integrateTsAst_1.collateInterfaceAst(tplName, tplBody, interfacePromiseTsAsTpl_1.interfacePromiseAst);
};
/**
...
exports.createPromiseParameters = (paramsTable) => {
    ...
    paramsTable.cells.forEach(value => {
        const singleParam = objDeepCopy_1.objDeepCopy(interfacePromiseTsAsTpl_1.interfacePromiseAst.declaration.body.body[0].parameters[0]);
